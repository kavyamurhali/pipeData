d3.parcoords = function (config) {
  ...
  w = function () {
    ...
    h = function () {
      __.height = parseFloat(parseFloat($("#analysis").height()));
  ...
  pc.autoscale = function () {
    ...
    var defaultScales = {
      ...
      "number": function (k) {
        ...
        var ty = d3.extent(__.data, function (d) {
          return +d[k];
        });
        ///Issue #46 start here 
        var count_val = Count_Ticks_Array.filter(item => item[k])[0][k];

        if (ty[0] == ty[1] || (k.includes('year') || k.includes('Year')) || count_val.length < 71) {
    ...
    // ctx.foreground.lineWidth = 1.3;
    // ctx.foreground.globalCompositeOperation = __.composite;
    // ctx.foreground.globalAlpha = __.alpha;
    //ctx.highlight.lineWidth = 1.4;

    ctx.foreground.strokeStyle = "rgba(0,100,160,0.1)";
    ctx.foreground.lineWidth = 1;
    ctx.foreground.globalCompositeOperation = __.composite;
    ctx.foreground.globalAlpha = __.alpha;
    ctx.highlight.lineWidth = 1.3;
  ...
  function compute_control_points(centroids) {
    ...
    cps.push([centroids[0][0] + a * 2 * (centroids[1][0] - centroids[0][0]), centroids[0][1]]);
    ...
    for (var col = 1; col < cols - 1; ++col) {
      ...
      var diff = [left[0] - right[0], left[1] - right[1]];


      cps.push([mid[0] + (diff[0] * a), mid[1] + (diff[1] * a)]);
      cps.push(mid);
      cps.push([mid[0] - (diff[0] * a), mid[1] - (diff[1] * a)]);


    }
    cps.push([centroids[cols - 1][0] + a * 2 * (centroids[cols - 2][0] - centroids[cols - 1][0]), centroids[cols - 1][1]]);
  ...
  function path_foreground(d, i) {
    ctx.foreground.imageSmoothingQuality = "high";
    ctx.foreground.lineDashOffset = 10;
    ctx.foreground.lineJoin = "round";
  ...
  function path_highlight(d, i) {
    ctx.highlight.strokeStyle = Colors_Var;
  ...
  function rotateLabels() {
    ...
    pc.svg.selectAll("text.label")
      .attr("transform", "translate(0,-15) rotate(" + __.dimensionTitleRotation + ")");
    d3.event.preventDefault();
  }

  function keep_data(new_data) {
    ...
    __.data = new_data;
    Brushed_Clicked = true;
    DrawD3_Parallel(__.data);
  ...
  pc.createAxes = function () {
    ...
    g = pc.svg.selectAll(".dimension")
      ...
      .attr("class", "dimension").attr("leftpx", function (d) {
        return xscale(d);
      });
    CloseFun();
    ...
    g.append("svg:g")
      ...
      .attr({
        "text-anchor": "middle",
        "y": -10,
        "transform": "translate(0,-15) rotate(" + __.dimensionTitleRotation + ")",
        "x": 0,
        "class": "label attrtext"
      })
      .text(function (d) {
        return d in __.dimensionTitles ? __.dimensionTitles[d] : d; // dimension display names
      })
      .on("click", flipAxisAndUpdatePCP)
    ...
    g.style("cursor", "move")
      .call(d3.behavior.drag()
        ...
        .on("dragend", function (d, i) {
          ...
          delete dragging[d];
          d3.select(this).transition().attr("transform", "translate(" + xscale(d) + ")").attr("leftpx", xscale(d));
          pc.render();
  ...
  (function () {
    ...
    function selected() {
      ...
      var within = {
        ...
        "number": function (d, p, dimension) {
          ...
          var count_val = Count_Ticks_Array.filter(item => item[p])[0][p];

          if ((p.includes('year') || p.includes('Year')) || count_val.length < 71)
    ...
    ///by keeping data into array //#endregion
    ...
    function brushFor(axis) {
      ...
      brush
        ...
        .on("brushend", function () {
          ...
          setTimeout(() => {
            graph.color(Colors_Var);
            //Issue# 43 moved code down from brush to brushed line 970 to line 976
            brushUpdated(selected());
            events.brushend.call(pc, __.brushed);
            keep_data(__.brushed);
          }, 200);
  ...
  pc.resize = function () {
    if (!$('.progress').is(':visible')) {
    Progress_reset(1);
    $protext.html('Resizing');
    }
    // selection size
    __.width = parseFloat($("#analysis").width());
    __.height = parseFloat($("#analysis").height());
    ...
    // if (flags.brushable){
    //   pc.brushable();
    //   graph.brushMode("1D-axes").brushPredicate('AND');
    // } 
    // if (flags.reorderable) pc.reorderable();
